...
const DrawerContent = (props) => {
  const { user, restaurant, logout, changeRestaurantStatus, toggleDineInStatus } = useAuth();
        ...
        <View style={styles.drawerContent}>
          <View style={styles.userInfoSection}>
            <View style={{ flexDirection: 'row', marginTop: 15 }}>
              <Avatar.Image
                source={{ uri: restaurant?.restaurantLogo || 'https://via.placeholder.com/150' }}
                size={50}
              />
              <View style={{ marginLeft: 15, flexDirection: 'column' }}>
                <Title style={styles.title}>{restaurant?.restaurantName || 'Restaurant Name'}</Title>
                <Caption style={styles.caption}>{user?.email || 'email@example.com'}</Caption>
              </View>
            </View>
          </View>
          ...
          <Drawer.Section style={styles.drawerSection}>
            ...
            <DrawerItem
              ...
              label="Waiter Calls"
              onPress={() => {
                props.navigation.navigate('WaiterCalls');
              }}
            />
            <DrawerItem
              icon={({ color, size }) => (
          ...
          <Drawer.Section title="Restaurant Status">
            <TouchableRipple onPress={handleRestaurantStatusToggle}>
              <View style={styles.preference}>
                ...
                <View pointerEvents="none">
                  <Switch value={isRestaurantOnline} color={COLORS.primary} />
                </View>
              </View>
            </TouchableRipple>
            ...
            <TouchableRipple onPress={handleDineInToggle}>
              <View style={styles.preference}>
                ...
                <View pointerEvents="none">
                  <Switch value={isDineInAvailable} color={COLORS.primary} />
                </View>
              </View>
            </TouchableRipple>
          </Drawer.Section>
