...
const DashboardScreen = () => {
  ...
  const { pendingOrders, processingOrders, activeDineIn, loadOrders, loadActiveDineIn, isLoading: ordersLoading } = useOrder();
  const { waiterCalls, loadWaiterCalls, acknowledgeWaiterCall, resolveWaiterCall, isLoading: waiterCallsLoading } = useWaiterCall();
      ...
      <Card containerStyle={styles.orderCard}>
        <View style={styles.orderHeader}>
          ...
          <Badge 
            ...
            status={item.orderStatus === 'pending' ? 'warning' : 'primary'} 
      ...
      <Card containerStyle={styles.waiterCallCard}>
        <View style={styles.waiterCallHeader}>
          ...
          <Badge 
            ...
            status={item.status === 'pending' ? 'warning' : 'primary'} 
            containerStyle={styles.badge}
          />
        </View>
        ...
        <View style={styles.waiterCallInfo}>
          <Text style={styles.waiterCallInfoText}>
            <Text style={styles.waiterCallInfoLabel}>Customer: </Text>
            {item.customerName || 'N/A'}
          </Text>
          {item.message ? (
            <Text style={styles.waiterCallInfoText}>
              <Text style={styles.waiterCallInfoLabel}>Message: </Text>
              {item.message}
            </Text>
          ) : null}
          <Text style={styles.waiterCallInfoText}>
        ...
        <View style={styles.waiterCallActions}>
          {item.status === 'pending' && (
            <TouchableOpacity 
              style={[styles.waiterCallButton, styles.acknowledgeButton]}
              onPress={() => acknowledgeWaiterCall(item.callId)}
            >
              <Text style={styles.waiterCallButtonText}>Acknowledge</Text>
            </TouchableOpacity>
          )}
          
          {item.status !== 'resolved' && (
            <TouchableOpacity 
              style={[styles.waiterCallButton, styles.resolveButton]}
              onPress={() => resolveWaiterCall(item.callId)}
            >
              <Text style={styles.waiterCallButtonText}>Resolve</Text>
            </TouchableOpacity>
          )}
        </View>
      </Card>
      ...
      >
        <View style={styles.statusContainer}>
          <Card containerStyle={styles.statusCard}>
            ...
            <TouchableOpacity 
              style={[
                styles.statusButton, 
                restaurantStatus === 'online' ? styles.onlineButton : styles.offlineButton
              ]}
              onPress={toggleRestaurantStatus}
            >
              <Text style={styles.statusButtonText}>
                {restaurantStatus === 'online' ? 'Online' : 'Offline'}
              </Text>
            </TouchableOpacity>
          </Card>
          ...
          <Card containerStyle={styles.statusCard}>
            ...
            <TouchableOpacity 
              style={[
                styles.statusButton, 
                dineInStatus ? styles.onlineButton : styles.offlineButton
              ]}
        ...
        <Card containerStyle={styles.tabsCard}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            ...
            >
              <Text style={[styles.tabButtonText, activeTab === 'tab1' && styles.activeTabButtonText]}>
                Order Received ({pendingOrders.length})
              </Text>
            </TouchableOpacity>
            
            <TouchableOpacity 
              style={[styles.tabButton, activeTab === 'tab3' && styles.activeTabButton]}
              onPress={() => selectTab('tab3')}
            >
              <Text style={[styles.tabButtonText, activeTab === 'tab3' && styles.activeTabButtonText]}>
            ...
            >
              <Text style={[styles.tabButtonText, activeTab === 'tab4' && styles.activeTabButtonText]}>
                Table Status ({activeDineIn.length})
              </Text>
            </TouchableOpacity>
            
            <TouchableOpacity 
              style={[styles.tabButton, activeTab === 'tab5' && styles.activeTabButton]}
              onPress={() => selectTab('tab5')}
            >
              <Text style={[styles.tabButtonText, activeTab === 'tab5' && styles.activeTabButtonText]}>
          ...
          {activeTab === 'tab1' && (
            <View>
              {ordersLoading ? (
                ...
                pendingOrders.map((order) => renderOrderItem(order))
              ) : (
          ...
          {activeTab === 'tab3' && (
            <View>
              {ordersLoading ? (
                ...
                processingOrders.map((order) => renderOrderItem(order))
              ) : (
          ...
          {activeTab === 'tab4' && (
            <View>
              {ordersLoading ? (
                ...
                activeDineIn.map((dineIn) => renderDineInItem(dineIn))
              ) : (
          ...
          {activeTab === 'tab5' && (
            <View>
              {waiterCallsLoading ? (
                ...
                waiterCalls.map((call) => renderWaiterCallItem(call))
              ) : (
                <Text style={styles.emptyText}>There are currently no waiter calls.</Text>
              )}
            </View>
          )}
        </View>
